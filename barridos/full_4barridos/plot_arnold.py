...
for direction in directions:
    with open(f"../results_barridos/{direction}/periodsgrid_total.npy", "rb") as f:
        barridos[direction] = np.load(f)
...
Tausrange = np.linspace(minRs, maxRs, dimRs)*0.1
...
cm1 = ax1.pcolormesh(kk, tt, barridos["right"], cmap=cmap, vmin=0.5, vmax=8.5)
cm2 = ax2.pcolormesh(kk, tt, barridos["left"], cmap=cmap, vmin=0.5, vmax=8.5)
cm3 = ax3.pcolormesh(kk, tt, barridos["up"], cmap=cmap, vmin=0.5, vmax=8.5)
cm4 = ax4.pcolormesh(kk, tt, barridos["down"], cmap=cmap, vmin=0.5, vmax=8.5)


for ax in [ax1, ax3]:
    #  ax.set_xlabel("K", size=24)
    ax.set_ylabel(r"$I_{in}$ (mA)", size=24)
for ax in [ax3, ax4]:
    ax.set_xlabel(r"$\tau_s$ (ms)", size=24)
for ax in [ax1, ax2, ax3, ax4]:
    ax.tick_params(axis='both', which='major', labelsize=24)
    ax.tick_params(axis='both', which='minor', labelsize=24)
ax1.set_title(r"$\tau_s$ ascendente", fontsize=24)
ax2.set_title(r"$\tau_s$ descendente", fontsize=24)
ax3.set_title(r"$I_{in}$ ascendente", fontsize=24)
ax4.set_title(r"$I_{in}$ descendente", fontsize=24)
...
cbar = fig.colorbar(cm1, ticks=range(1,9), cax=cbar_ax)
...
for n in range(1,n):
    pgsectors = np.zeros_like(barridos["right"])
    for periods_grid in barridos.values():
    ...
    aaa = ax.contour(kk, tt, pgsectors, levels=[n-eps, n+eps], colors=[colors[n-1], "#000"], linewidths=[2,0], alpha=0.8)
...
ax.set_xlabel(r"$\tau_s$ (ms)", size=size)
ax.set_ylabel(r"$I_{in}$ (mA)", size=size)
