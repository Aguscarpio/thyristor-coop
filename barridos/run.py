...
parser.add_argument('-direction', type=str, nargs=1, help='right or left', default=["right"])
...
if direction in ["right", "left"]:
    target_program = "arnold_n_horizontal.py"
elif direction in ["up", "down"]:
    target_program = "arnold_n_vertical.py"

if not os.path.isdir(f"results_barridos/{str(direction)}"):
    os.system(f"mkdir results_barridos/{str(direction)}")

for arn_number in range(ncpus):
    os.system(f"python3 {target_program} {arn_number} {ncpus}" +
            f" {dimRs} {dimIin} {N_steps} {dt} {Cm} {direction} &")
